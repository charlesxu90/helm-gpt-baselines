...
from .afp import renumber_frag_attachment_idxs, match_fragment_attachment_points
from .fragmentors import FragmentorBase
from .gene_type_utils import get_gene_type, get_haplotype_from_gene_frag
...
def add_node_mutation(parent_mol: Chem.rdchem.Mol, fragmentor: FragmentorBase,
    ...
    while (accept_mutant is False) and (len(cut_types) > 0):
        ...
        mutant_gene_type_list = sorted([int(x) for x in gene_type_list if x != ''] + [int(added_cut_type2)])
