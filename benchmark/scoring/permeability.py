...
class Permeability:
    ...
    """
    def __init__(self, model_path='data/cpp/regression_rf.pkl', predictor_name='rf', batch_size=1000):
        self.predictor = self.load_predictor(model_path, predictor_name=predictor_name)
        self.trans_fn = TransformFunction('sigmoid', -8, -6, params={'k': 1.})  # high: > -6, low < -6
        self.batch_size = batch_size

    @staticmethod
    def load_predictor(model_path, predictor_name='rf'):
        # logger.debug(f'Predictor {predictor_name} -----')
        predictor = joblib.load(model_path)
        return predictor

    @staticmethod
    def get_features(helm_seqs: list):
        valid_smiles, valid_idxes = get_smi_from_helms(helm_seqs)
    ...
    def __call__(self, helm_seqs: list):
        scores = self.get_scores(helm_seqs)
        scores_tfd = self.trans_fn(scores)
        return scores_tfd, scores
    
    def get_scores(self, helm_seqs: list):
        scores = -10 * np.ones(len(helm_seqs))
        valid_features, valid_idxes = self.get_features(helm_seqs)
