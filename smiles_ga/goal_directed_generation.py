...
from benchmark.assess_goal_directed_generation import assess_goal_directed_generation
from benchmark.goal_directed_generator import GoalDirectedGenerator
...
def mutate(p_gene, scoring_function):
    ...
    try:
        ...
        # logger.debug(f'Mutant smiles: {c_smiles}')
        c_score = scoring_function.score(c_smiles)
        # logger.debug(f'Mutant score: {c_score}')
    except:
        c_smiles = None
        c_score = None
...
class ChemGEGenerator(GoalDirectedGenerator):
    ...
    def __init__(self, smi_file, population_size, n_mutations, gene_size, generations, n_jobs=-1, random_start=False, patience=5):
        ...
        logger.debug(f'Loading smiles from {smi_file}')
    ...
    def generate_optimized_molecules(self, scoring_function, number_molecules: int,
        ...
        if starting_population is None:
            logger.info('selecting initial population...')
        ...
        # logger.debug(f'Initial population: {len(starting_population)}')
        ...
        initial_genes = [cfg_to_gene(cfg_util.encode(s), max_len=self.gene_size) for s in starting_population]
        ...
        for generation in range(self.generations):
            ...
            # logger.debug(f'Generation {generation}: {len(genes_to_mutate)} genes to mutate')
            ...
            # logger.debug(f'Generation {generation}: {len(new_population)} new molecules')

            # join and dedup
            population += new_population
            population = deduplicate(population)
            # logger.debug(f'Generation {generation}: {len(population)} unique molecules')
            ...
            population_scores = [p.score for p in population]
            # logger.debug(f'Generation {generation}: max score {np.max(population_scores):.3f}')

            # early stopping
            if population_scores == old_scores:
                patience += 1
                logger.info(f'Failed to progress: {patience}')
                if patience >= self.patience:
                    logger.info(f'No more patience, bailing...')
                    break
            else:
                patience = 0
